...
const applySnakePosition = prevState => {
  ...
  const snakeTail =
    isSnakeEating ? prevState.snake.coordinates :
    getSnakeWithoutStub(prevState.snake);

  const snackCoordinate =
...
class App extends Component {
  ...
  onTick = () => {
      ...
      getIsSnakeOutside(this.state.snake) || getIsSnakeClumy(this.state.snake) ? this.setState(
        applyGameOver,
      ) :
...
const Grid = ({ isGameOver, snake, snack }) => (
  <div>
    {GRID.map(y => <Row y={y} key={y} snake={snake} snack={snack} isGameOver={isGameOver} />)}
  </div>
);
...
const Row = ({ isGameOver, snake, snack, y }) => (
  <div className='grid-row'>
    {GRID.map(x => (
      <Cell x={x} y={y} key={x} snake={snake} snack={snack} isGameOver={isGameOver} />
    ))}
  </div>
);
